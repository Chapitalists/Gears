...
update msg (Model model) =
    ...
    (case msg of
     ...
     --                | model =
     --                    { model
     --                        | loadedSoundList = s :: model.loadedSoundList
     --                    }
     --            }
     --
     --ClickedUploadSound ->
...
--
--                    "cut" ->
--                        D.map (Cuted << Sound.fileNameFromPath) <|
...
--isValidFile file =
--    List.member (File.mime file) soundMimeTypes
--        && (File.size file <= (200 * 1024 * 1024))


viewFullLib : Size -> Float -> Library -> Element Msg
viewFullLib screenSize scale (Model model) =
    wrappedRow
        [ Bg.color (toEl bgBase)
        , width <| px screenSize.width
        ]
    <|
        List.map (viewSoundInFull scale (screenSize.width // 8)) <|
            Dict.toList model.files


viewSoundInFull : Float -> Int -> ( String, FileInfo ) -> Element Msg
viewSoundInFull scale maxSize ( path, { durSec } ) =
    Input.button
        [ width <| px maxSize
        , height <| px maxSize
        , centerX
...
--        [ column [ spacing 20 ] <|
--            viewOpenRefreshButtons ClickedUploadSound
--                RequestSoundList
--                model.serverResponding
--        , viewLibColumn <| viewLib model [] model.files
--        ]
--    ]
...
--                filterOut =
--                    not <|
--                        String.contains (String.toLower model.fileFilter) <|
--                            String.toLower <|
--                                String.join "/" newId
--            in
--            case sType of
...
--        List.sortWith
--            (\t1 t2 ->
--                Natural.compare (Tuple.first t1) (Tuple.first t2)
--            )
--        <|
--            Dict.toList dict
--
...
--            [ Font.color <|
--                if
--                    List.any ((==) <| String.join "/" id) <|
--                        List.map Sound.getPath model.loadedSoundList
--                then
--                    rgb 0.2 0.8 0.2
--
...
--                List.sortWith
--                    (\s t -> Natural.compare (Sound.getPath s) (Sound.getPath t))
--                <|
--                    filterFiles model.fileFilter
--                        Sound.getPath
--                        model.loadedSoundList
--            )
--        ]
...
--checkLoad :
--    List Sound
--    -> Content.Content Wheel
--    -> SoundList
--    -> List (Cmd msg)
--    -> Maybe ( Content.Content Wheel, SoundList, List (Cmd msg) )
--checkLoad loaded content soundLib cmds =
--    case content of
--        Content.S s ->
--            if List.member s loaded then
...
--                        case mayAcc of
--                            Just ( coll, sl, cc ) ->
--                                case checkLoad loaded (Wheel.getContent g) sl cc of
...
--                        case mayAcc of
--                            Just ( collar, sl, cc ) ->
--                                case checkLoad loaded (Wheel.getContent b) sl cc of
--                                    Just ( newContent, newSL, newCmds ) ->
